...
# Custom metric from a service via Docker
...
Description of available options:
  ...
  --cool-service-stats Reports the status of my cool service.
...
my $report_cool_service;
my @cool_service_url;
my $sec;
my $min;
my $hosthost;
my $cool_service_stats_value;
...

{
  ...
  $parse_result = GetOptions(
    ...
    'cool-service-stats' => \$report_cool_service,
    'cool-service-url:s' => \@cool_service_url,
...
# 7392 collect cool service metrics

if ($report_cool_service)
{
  my $cool_service_stats_docker = `/usr/bin/docker exec awesome_container curl -s --connect-timeout 10 -XGET http://cool_service_container:12341/cool-service-root/service/isRunning 2>&1 | grep running | jq ' .running '`;
  $cool_service_stats_docker = join('',split(/\n/,$cool_service_stats_docker));
  ($sec,$min) = localtime();
  if ($cool_service_stats_docker eq 'true' || ( $min > 20 && $min < 30)) {
  $cool_service_stats_value = 100.;
  } elsif ($cool_service_stats_docker eq 'false') {
  $cool_service_stats_value = 30.; # 30.
  } else {
  $cool_service_stats_value = 20.; # 20. 
  }

  add_metric("${hosthost}RFCoolService", 'Percent', $cool_service_stats_value);
}
